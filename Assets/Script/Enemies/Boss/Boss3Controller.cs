...
public class Boss3Controller : MonoBehaviour, IDamageable
{
    ...
    private void Awake()
    {
        ...
        // REKOMENDASI: Cari komponen serangan secara otomatis untuk mengurangi risiko error
        // jika referensi tidak di-drag ke Inspector.
        if (laserSpread == null)
        {
            laserSpread = GetComponentInChildren<Boss3LaserSpread>();
        }
        if (laserSpiral == null)
        {
            laserSpiral = GetComponentInChildren<Boss3LaserSpiral>();
        }
    ...
    private void PerformAttack()
    {
        switch (currentPattern)
        {
            case 0:
                ...
                {
                    Debug.LogError("Referensi 'laserSpread' pada Boss3Controller hilang! Serangan spread tidak bisa dilakukan.", this);
                }
                break;

            case 1:
                if (spiralCoroutine == null && laserSpiral != null)
                {
                    spiralCoroutine = StartCoroutine(FireSpiralTemporarily());
                }
                else if (laserSpiral == null)
                {
                    Debug.LogError("Referensi 'laserSpiral' pada Boss3Controller hilang! Serangan spiral tidak bisa dilakukan.", this);
                }
                break;
        }
